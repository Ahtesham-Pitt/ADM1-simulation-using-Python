...
def ADM1_model(t, x):
    ...
    stoich6 = -C_aa + (1.0 - Y_aa) * (
                f_va_aa * C_va + f_bu_aa * C_bu + f_pro_aa * C_pro + f_ac_aa * C_ac) + Y_aa * C_bac
    ...
    reac[5] = (1.0 - Y_su) * f_pro_su * proc5 + (1.0 - Y_aa) * f_pro_aa * proc6 + (
                1.0 - Y_c4) * 0.54 * proc8 - proc10  # S_pro
    reac[6] = (1.0 - Y_su) * f_ac_su * proc5 + (1.0 - Y_aa) * f_ac_aa * proc6 + (1.0 - Y_fa) * 0.7 * proc7 + (
                1.0 - Y_c4) * 0.31 * proc8 + (1.0 - Y_c4) * 0.8 * proc9 + (1.0 - Y_pro) * 0.57 * proc10 - proc11  # S_ac
    reac[7] = (1.0 - Y_su) * f_h2_su * proc5 + (1.0 - Y_aa) * f_h2_aa * proc6 + (1.0 - Y_fa) * 0.3 * proc7 + (
                1.0 - Y_c4) * 0.15 * proc8 + (1.0 - Y_c4) * 0.2 * proc9 + (
                          1.0 - Y_pro) * 0.43 * proc10 - proc12 - procT8  # S_h2
    reac[8] = (1.0 - Y_ac) * proc11 + (1.0 - Y_h2) * proc12 - procT9  # S_ch4
    reac[
        9] = -stoich1 * proc1 - stoich2 * proc2 - stoich3 * proc3 - stoich4 * proc4 - stoich5 * proc5 - stoich6 * proc6 - stoich7 * proc7 - stoich8 * proc8 - stoich9 * proc9 - stoich10 * proc10 - stoich11 * proc11 - stoich12 * proc12 - stoich13 * proc13 - stoich13 * proc14 - stoich13 * proc15 - stoich13 * proc16 - stoich13 * proc17 - stoich13 * proc18 - stoich13 * proc19 - procT10  # S_IC
    reac[10] = (N_xc - f_xI_xc * N_I - f_sI_xc * N_I - f_pr_xc * N_aa) * proc1 - Y_su * N_bac * proc5 + (
                N_aa - Y_aa * N_bac) * proc6 - Y_fa * N_bac * proc7 - Y_c4 * N_bac * proc8 - Y_c4 * N_bac * proc9 - Y_pro * N_bac * proc10 - Y_ac * N_bac * proc11 - Y_h2 * N_bac * proc12 + (
                           N_bac - N_xc) * (proc13 + proc14 + proc15 + proc16 + proc17 + proc18 + proc19)  # S_IN
...
t_span = [0, 200]  # days
y0 = 6 * DIGESTERINIT  # initial conditions
...
x_ss_BSM2 = [0.0123944521960000, 0.00554315146500000, 0.107407097456000, 0.0123325298170000, 0.0140030358520000,
             ...
             0.0944681918740000, 0.130867066076000, 0.107920890509000, 0.0205168226890000, 0.0842203691100000,
             0.0436287448300000, 0.312223348554000, 0.931671811511000, 0.338391125445000, 0.335772136662000,
             0.101120522211000, 0.677244333548000, 0.284839619000000, 17.2162246904400, 0, 0.00521009922300000,
             0.0122839755570000, 0.0139527381860000, 0.0175114392780000, 0.0890351618480000, 0.0856799661500000,
             0.00188401134000000, 1.10324120000000e-05, 1.65349847162100, 0.0135401278050000, 2708.343137271645]
...
def ADM1_balance(x):
    ...
    data["Ssu"] = {"in": u[26] * u[0],
                   "Hydrolysis of Carbohydrates": proc2 * V_liq,
                   "Hydrolysis of Lipids": (1.0 - f_fa_li) * proc4 * V_liq,
                   "Uptake of Sugars": - proc5 * V_liq, "out": u[26] * x[0]}

    data["Saa"] = {"in": u[26] * u[1],
                   "Hydrolysis of Proteins": proc3 * V_liq,
                   "Uptake of Amino Acids": -proc6 * V_liq, "out": u[26] * x[1]}

    data["Sfa"] = {"in": u[26] * u[2],
                   "Hydrolysis of Lipids": f_fa_li * proc4 * V_liq,
                   "Uptake of LCFA": - proc7 * V_liq, "out": u[26] * x[2]}

    data["Sva"] = {"in": u[26] * u[3],
                   "Uptake of Amino Acids": (1.0 - Y_aa) * f_va_aa * proc6 * V_liq,
                   "Uptake of Valerate": - proc8 * V_liq, "out": u[26] * x[3]}

    data["Sbu"] = {"in": u[26] * u[4],
                   "Uptake of Sugars": (1.0 - Y_su) * f_bu_su * proc5 * V_liq,
                   "Uptake of Amino Acids": (1.0 - Y_aa) * f_bu_aa * proc6 * V_liq,
                   "Uptake of Butyrate": - proc9*V_liq, "out": u[26] * x[4]}

    data["Spro"] = {"in": u[26] * u[5],
                    "Uptake of Sugars": (1.0 - Y_su) * f_pro_su * proc5 * V_liq,
                    "Uptake of Amino Acids": (1.0 - Y_aa) * f_pro_aa * proc6 * V_liq,
                    "Uptake of Valerate": (1.0 - Y_c4) * 0.54 * proc8*V_liq,
                    "Uptake of Propionate": - proc10*V_liq, "out": u[26] * x[5]}

    data["Sac"] = {"in": u[26] * u[6],
                   "Uptake of Sugars": (1.0 - Y_su) * f_ac_su * proc5 * V_liq,
                   "Uptake of Amino Acids": (1.0 - Y_aa) * f_ac_aa * proc6 * V_liq,
                   "Uptake of LCFA": (1.0 - Y_fa) * 0.7 * proc7 * V_liq,
                   "Uptake of Valerate": (1.0 - Y_c4) * 0.31 * proc8 * V_liq,
                   "Uptake of Butyrate": (1.0 - Y_c4) * 0.8 * proc9 * V_liq,
                   "Uptake of Propionate": (1.0 - Y_pro) * 0.57 * proc10 * V_liq,
                   "Uptake of Acetate": - proc11*V_liq, "out": u[26] * x[6]}

    data["Sh2"] = {"in": u[26] * u[7],
                   "Uptake of Sugars": (1.0 - Y_su) * f_h2_su * proc5 * V_liq,
                   "Uptake of Amino Acids": (1.0 - Y_aa) * f_h2_aa * proc6 * V_liq,
                   "Uptake of LCFA": (1.0 - Y_fa) * 0.3 * proc7 * V_liq,
                   "Uptake of Valerate": (1.0 - Y_c4) * 0.15 * proc8 * V_liq,
                   "Uptake of Butyrate": (1.0 - Y_c4) * 0.2 * proc9 * V_liq,
                   "Uptake of Propionate": (1.0 - Y_pro) * 0.43 * proc10 * V_liq,
                   "Uptake of Hydrogen": - proc12 * V_liq,
                   "H2 Transfer": - procT8*V_liq, "out": u[26] * x[7]}

    data["Sch4"] = {"in": u[26] * u[8],
                    "Uptake of Acetate": (1.0 - Y_ac) * proc11 * V_liq,
                    "Uptake of Hydrogen": (1.0 - Y_h2) * proc12 * V_liq,
                    "CH4 Transfer": - procT9 * V_liq, "out": u[26] * x[8]}

    data["SIC"] = {"in": u[26] * u[9],
                   "Disintegration": -stoich1 * proc1 * V_liq,
                   "Hydrolysis of Carbohydrates": - stoich2 * proc2 * V_liq,
                   "Hydrolysis of Proteins": - stoich3 * proc3 * V_liq,
                   "Hydrolysis of Lipids": - stoich4 * proc4 * V_liq,
                   "Uptake of Sugars": - stoich5 * proc5 * V_liq,
                   "Uptake of Amino Acids": - stoich6 * proc6 * V_liq,
                   "Uptake of LCFA": - stoich7 * proc7*V_liq,
                   "Uptake of Valerate": - stoich8 * proc8 * V_liq,
                   "Uptake of Butyrate": - stoich9 * proc9 * V_liq,
                   "Uptake of Propionate": - stoich10 * proc10 * V_liq,
                   "Uptake of Acetate": - stoich11 * proc11 * V_liq,
                   "Uptake of Hydrogen": - stoich12 * proc12 * V_liq,
                   "Decay of Xsu": - stoich13 * proc13 * V_liq,
                   "Decay of Xaa": - stoich13 * proc14 * V_liq,
                   "Decay of Xfa": - stoich13 * proc15 * V_liq,
                   "Decay of Xch4": - stoich13 * proc16 * V_liq,
                   "Decay of Xpro": - stoich13 * proc17 * V_liq,
                   "Decay of Xac": - stoich13 * proc18 * V_liq,
                   "Decay of Xh2": - stoich13 * proc19 * V_liq,
                   "CO2 Transfer": - procT10 * V_liq, "out": u[26] * x[9]}

    data["SIN"] = {"in": u[26] * u[10],
                   "Disintegration": (N_xc - f_xI_xc * N_I - f_sI_xc * N_I - f_pr_xc * N_aa) * proc1 * V_liq,
                   "Uptake of Sugars": - Y_su * N_bac * proc5 * V_liq,
                   "Uptake of Amino Acids": (N_aa - Y_aa * N_bac) * proc6 * V_liq,
                   "Uptake of LCFA": - Y_fa * N_bac * proc7 * V_liq,
                   "Uptake of Valerate": - Y_c4 * N_bac * proc8 * V_liq,
                   "Uptake of Butyrate": - Y_c4 * N_bac * proc9 * V_liq,
                   "Uptake of Propionate": - Y_pro * N_bac * proc10 * V_liq,
                   "Uptake of Acetate": - Y_ac * N_bac * proc11 * V_liq,
                   "Uptake of Hydrogen": - Y_h2 * N_bac * proc12 * V_liq,
                   "Decay of Xsu": (N_bac - N_xc) * proc13 * V_liq,
                   "Decay of Xaa": (N_bac - N_xc) * proc14 * V_liq,
                   "Decay of Xfa": (N_bac - N_xc) * proc15 * V_liq,
                   "Decay of Xch4": (N_bac - N_xc) * proc16 * V_liq,
                   "Decay of Xpro": (N_bac - N_xc) * proc17 * V_liq,
                   "Decay of Xac": (N_bac - N_xc) * proc18 * V_liq,
                   "Decay of Xh2": (N_bac - N_xc) * proc19 * V_liq, "out": u[26] * x[10]}

    data["SI"] = {"in": u[26] * u[11],
                  "Disintegration": f_sI_xc * proc1 * V_liq, "out": u[26] * x[11]}

    data["Xc"] = {"in": u[26] * u[12],
                  "Disintegration": -proc1 * V_liq,
                  "Decay of Xsu": proc13 * V_liq,
                  "Decay of Xaa": proc14 * V_liq,
                  "Decay of Xfa": proc15 * V_liq,
                  "Decay of Xch4": proc16 * V_liq,
                  "Decay of Xpro": proc17 * V_liq,
                  "Decay of Xac": proc18 * V_liq,
                  "Decay of Xh2": proc19 * V_liq, "out": u[26] * x[12]}

    data["Xch"] = {"in": u[26] * u[13],
                   "Disintegration":f_ch_xc * proc1*V_liq,
                   "Hydrolysis of Carbohydrates": - proc2*V_liq, "out": u[26] * x[13]}

    data["Xpr"] = {"in": u[26] * u[14], "Disintegration": f_pr_xc * proc1*V_liq,
                   "Hydrolysis of Proteins": - proc3*V_liq,"out": u[26] * x[14]}

    data["Xli"] = {"in": u[26] * u[15], "Disintegration": f_li_xc * proc1*V_liq,
                   "Hydrolysis of Lipids": - proc4*V_liq, "out": u[26] * x[15]}

    data["Xsu"] = {"in": u[26] * u[16], "Uptake of Sugars": Y_su * proc5*V_liq,
                   "Decay of Xsu": - proc13*V_liq, "out": u[26] * x[16]}

    data["Xaa"] = {"in": u[26] * u[17],
                   "Uptake of Amino Acids": Y_aa * proc6*V_liq,
                   "Decay of Xaa": - proc14*V_liq, "out": u[26] * x[17]}

    data["Xfa"] = {"in": u[26] * u[18],
                   "Uptake of LCFA": Y_fa * proc7*V_liq,
                   "Decay of Xfa":- proc15*V_liq,"out": u[26] * x[18]}

    data["Xch4"] = {"in": u[26] * u[19],
                    "Uptake of Valerate": Y_c4 * proc8*V_liq,
                    "Uptake of Butyrate": Y_c4 * proc9*V_liq,
                    "Decay of Xch4": - proc16*V_liq, "out": u[26] * x[19]}

    data["Xpro"] = {"in": u[26] * u[20],
                    "Uptake of Propionate": Y_pro * proc10*V_liq,
                    "Decay of Xpro":- proc17*V_liq, "out": u[26] * x[20]}

    data["Xac"] = {"in": u[26] * u[21], "Uptake of Acetate": Y_ac * proc11*V_liq,
                   "Decay of Xac": - proc18*V_liq, "out": u[26] * x[21]}

    data["Xh2"] = {"in": u[26] * u[22], "Uptake of Hydrogen": Y_h2 * proc12*V_liq,
                   "Decay of Xh2": - proc19*V_liq, "out": u[26] * x[22]}

    data["XI"] = {"in": u[26] * u[23],
                  "Disintegration": f_xI_xc * proc1*V_liq, "out": u[26] * x[23]}

    df = pd.DataFrame(data)
    custom_order = ["in", "Disintegration", "Hydrolysis of Carbohydrates", "Hydrolysis of Proteins",
                    "Hydrolysis of Lipids", "Uptake of Sugars", "Uptake of Amino Acids", "Uptake of LCFA",
                    "Uptake of Valerate", "Uptake of Butyrate", "Uptake of Propionate", "Uptake of Acetate",
                    "Uptake of Hydrogen", "Decay of Xsu", "Decay of Xaa", "Decay of Xfa", "Decay of Xch4",
                    "Decay of Xpro", "Decay of Xac", "Decay of Xh2", "H2 Transfer", "CH4 Transfer", "CO2 Transfer", "out"]
    df = df.reindex(custom_order)
    df.loc["out"] = -df.loc["out"]

    # Save the dataframe to an Excel file in the current directory
    df.to_excel("ADM1_COD_matrix.xlsx")
...
dx, df = ADM1_balance(x_ss_python)
# Replace NaN with 0
df = df.fillna(0)
df_ex_out = df.drop(index=["out"])
df_out = df.loc["out"]
df_ex_out.sum() - df_out